...
### Create a namespace called 'mynamespace' and a pod with image nginx called nginx on this namespace (DONE)
...
### Create the pod that was just described using YAML (DONE)
...
### Create a busybox pod (using kubectl command) that runs the command "env". Run it and see the output (DONE)
...
### Get the YAML for a new ResourceQuota called 'myrq' with hard limits of 1 CPU, 1G memory and 2 pods without creating it (DONE)
...
### Get nginx pod's ip created in previous step, use a temp busybox image to wget its '/' (DONE)
...
### If pod crashed and restarted, get logs about the previous instance (DONE)
